...
UbloxMessage* ublox_worker_i2c_transfer(UbloxMessage message_tx, uint8_t read_length) {
    ...
    if(!furi_hal_i2c_tx(
           ...
           message_tx.length,
           furi_ms_to_ticks(I2C_TIMEOUT_MS))) {
        FURI_LOG_I(TAG, "error reading from GPS");
        return NULL;
    }
    uint8_t* response = malloc(read_length);
...
void ublox_worker_read_pvt(UbloxWorker* ublox_worker) {
    ...
    if(frame_rx == NULL) {
        ...
        /*if (nav_pvt.flags & 0b10) {
      FURI_LOG_I(TAG, "differential solution");
    } else {
      FURI_LOG_I(TAG, "no differential used in solution");
      }*/
...
void ublox_worker_read_odo(UbloxWorker* ublox_worker) {
    Ublox* ublox = ublox_worker->context;
    ...
    if(frame_rx == NULL) {
        ...
        //FURI_LOG_I(TAG, "distance (m): %lu, totalDistance: %lu", nav_odo.distance, nav_odo.totalDistance);
        ublox_frame_free(frame_rx);
        ublox_worker->callback(UbloxWorkerEventDataReady, ublox_worker->context);
    }
}

/** Set the power mode to "Aggressive with 1Hz" and enable the odometer. */
...
bool ublox_worker_init_gps(UbloxWorker* ublox_worker) {
    ...
    // set odo mode to car for testing
