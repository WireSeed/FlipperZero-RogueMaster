...
static void data_display_draw_callback(Canvas* canvas, void* model) {
    ...
    if(m->state == DataDisplayGPSNotFound) {
        ...
        furi_string_printf(s, "%d/%d/'%d ", message.day, message.month, (message.year % 100));
        ...
        /*
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str(canvas, 85, 12, "Fix:");

    if (message.fixType == 0) {
      canvas_draw_str(canvas, 106, 12, "N");
    } else if (message.fixType == 1) {
      canvas_draw_str(canvas, 106, 12, "R");
    } else if (message.fixType == 2) {
      canvas_draw_str(canvas, 106, 12, "2D");
    } else if (message.fixType == 3) {
      canvas_draw_str(canvas, 106, 12, "3D");
    } else if (message.fixType == 4) {
      canvas_draw_str(canvas, 106, 12, "G+D");
    } else if (message.fixType == 5) {
      canvas_draw_str(canvas, 106, 12, "TO");
    }
    */
...
void data_display_set_nav_pvt_message(DataDisplayView* data_display, Ublox_NAV_PVT_Message message) {
    furi_assert(data_display);
    with_view_model(
        data_display->view, DataDisplayViewModel * model, { model->nav_pvt = message; }, true);
}

void data_display_set_nav_odo_message(DataDisplayView* data_display, Ublox_NAV_ODO_Message message) {
    furi_assert(data_display);
    with_view_model(
        data_display->view, DataDisplayViewModel * model, { model->nav_odo = message; }, true);
}
