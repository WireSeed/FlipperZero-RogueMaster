...
// void anim(const Icon** replace, const char* name, FuriString* path, File* file) {
    // do {
        // furi_string_printf(path, ICONS_FMT "/meta", CFW_SETTINGS()->asset_pack, name);
        ...
        // const char* pack = CFW_SETTINGS()->asset_pack;
...
// void icon(const Icon** replace, const char* name, FuriString* path, File* file) {
    // furi_string_printf(path, ICONS_FMT ".bmx", CFW_SETTINGS()->asset_pack, name);
    // if(storage_file_open(file, furi_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING)) {
...
// void swap(CfwAssets* x, FuriString* p, File* f) {
