...
static int32_t lfrfid_hitag_worker_emulate_thread(void* thread_context) {
    ...
    uint8_t carrier_threshold_on =
        5; //better to use the #define on top for this constant or make this a constant
    int8_t carrier_threshold_off =
        -5; //better to use the #define on top for this constant or make this a constant
                            ...
                            if(cmd != 0) { //|| periodcounter%62500 == 0){
                                ...
                                if(dmaLen > 0) { //switch to reply mode
                                    ...
                                    while(capData->capCounter - cmdIndex < wait1_periods) {
                                        furi_delay_us(
                                            40); //wait 5 more periods (datasheet says 204-213 periods, so waiting for 5 periods should land somewhere in this range)
                                    }
...
static int32_t lfrfid_hitag_worker_read_thread(void* thread_context) {
    ...
    while(1) {
            ...
            while(expectedBits > 0 && index < buffsize) {
                ...
                if(!varint_pair_unpack(
                    ...
                    lfrfid_hitag_worker_decoder_feed(
                        pulse, duration, memBlock, &bits, &Scount, &finished, currentProtocol);

                    //see if i have a valid bit sequence
                    if(finished) {
                        //bitsequence ready for falidation
                        if(bits ==
                           expectedBits) { //eligable bitsequence detected, accept it only after 3 consequtive readings
        ...
        if(!readSucces && backupCMD->CMDcount >= 7 && currPage > 0 && expectedBits > 0) {
            backupCMD->CMDcount =
                0; //required since in next loop there might not be a HT/TC trigger to trigger the yield ftion where cmd count is updated/reset
            dataCMD->CMDposition = 0;
            dataCMD->CMDsubposition = 0;
        }
        ...
        if(worker->DMAeventCount >
           1) { //more than 1 unprocessed DMA event (aka DMA buffer is not timely refilled)
